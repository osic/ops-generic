...
class AntiaffinityCheck(object):
      ...
      def decider(self):
          ...
          if self.args.check:
              ...
              if output and self.args.json:
                  ...
                  print "Anti-affinity rules violated in Server Group:", self.args.check
                  self.print_table(output)
              elif not output:
                  print "No anti-affinity rules violated for Server Group:", self.args.check
          if self.args.list:
               ...
               if output and self.args.json:
                   ...
                   print "Server Group", self.args.list,"empty or does not have an anti-affinity policy set."
          if self.args.all:
      ...
      def check_arguments(self):
          ...
          group.add_argument('--list', type=str,help='List instances and their hypervisors for a given Server Group')
      ...
      def print_table(self,output):
          ...
          """
          table = self.create_table(['Instance ID', 'Instance Name', 'Hypervisor'])
